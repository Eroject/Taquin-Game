...
void Insererprecesseur(Etat nouveauetat, List *pliste,noeud *nprecesseur){
    ...
    nouveaunoeaud->suivant=L1.debut;
    L1.debut=nouveaunoeaud;*/
...
void Inserer(noeud *noe, List *pliste){//inserer un noeud sur une file
    Etat nouveauetat=noe->etat;
    noeud *nprecesseur=noe->precesseur;
    Insererprecesseur(nouveauetat,pliste,nprecesseur);
}

void Insereretat (Etat nouveauetat, List *pliste){//inserer un etat sur une file
    noeud *nprecesseur=NULL;
    Insererprecesseur(nouveauetat,pliste,nprecesseur);
}


noeud* Extraire (List* listenoeuds){// extraire un noeud du file"listenoeus"
    noeud * temp;
...
void Tri(List *liste,Etat etatFinal) {
    ...
    int c=0;
    noeud *i, *j, *min , *temp;
    noeud *precedentmin ,*precedenti,*precedentj;
    precedenti=NULL;
    i = liste->debut;
    while (i != NULL)  {
        min = i;
...
void RechercheAetoile(Etat etatInitial,Etat etatFinal){
    ...
    while(listeNoeuds.debut!=NULL){
        ...
        if (etatSolution(etatCourant,etatFinal)){
            ...
            afficherchemin(noeudCourantp);
            printf("\n");
            printf("Succes ! Arrer sur l'etat Final a partir de l'etat:\n");
            afficherMatrice(etatInitial.tableau,noeudCourantp->etat.g);
            exit(0);
        }
            ...
            Insereretat(etatCourant, &NoeudsDejaTraites);
               //         printf("avant\n");
           // afficherlist(listeNoeuds,etatFinal);
