...
std::pair<int, int> poszero() {
    ...
    return { -1, -1 }; // Si la position de zéro n'est pas trouvée
}
...
void graphicPlate(sf::RenderWindow& window, bool win) {
    ...
    if (win) {
        ...
        // Positionnement du texte au milieu du haut de la fenêtre
        ...
        sf::Text textBottom("AJALE Saad _BENHADOU Lhcen", font); 
...
MenuOption display(sf::RenderWindow& window) {
    ...
    sf::Text rulesText("Règles", font, 50);
...
void displayRules(sf::RenderWindow& window) {
    ...
    sf::Text title("Règles du Taquin", font, 30);
    title.setFillColor(sf::Color::White);
    title.setPosition(50.f, 20.f);

    sf::Text rules1("Le Taquin est un jeu de puzzle où le but est de", font, 20);
    rules1.setFillColor(sf::Color::White);
    rules1.setPosition(50.f, 80.f);

    sf::Text rules2("réarranger les numéros dans l'ordre, en déplaçant ", font, 20);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                return; // Ne ferme pas la fenêtre principale, seulement la fenêtre des règles
            }
        }
...
int main() {
    ...
    etatInitial.g = 0;
    sf::RenderWindow menuWindow(sf::VideoMode(700, 680), "Le jeu du Taquin - AJALE SAAD - BENHADDOU");
